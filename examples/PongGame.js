class PongGame extends gia.Component {
	...
	_handlePaddleCollisions() {
		...
		if (hitPlayer || hitAI) {
			...
			const hitFactor =
				(this.ballY + this.options.ballSize / 2 - (paddleY + this.options.paddleHeight / 2)) / (this.options.paddleHeight / 2);
	...
	draw() {
		...
		// Draw Player Paddle
		this.ctx.fillRect(this.options.paddlePadding, this.playerY, this.options.paddleWidth, this.options.paddleHeight);
...
gia.register(PongGame, { priority: -50 });
