class FilterableListClassic extends gia.Component {
	constructor(element) {
		super(element);
	...
	mount() {
		...
		if (getComputedStyle(this.ref.container).position === 'static') {
			this.ref.container.style.position = 'relative';
		}

		if (this.ref.item.length === 0) {
			console.warn("FilterableListClassic component is missing items.");
			return;
		}
	...
	_applyFLIPAnimation(visibleItems, hiddenItems) {
		gia.measure(() => {
			...
			const containerRect = this.ref.container.getBoundingClientRect();
			
			for (let i = 0; i < this.ref.item.length; i++) {
				const item = this.ref.item[i];
				if (!item.hidden) {
			...
			gia.mutate(() => {
				...
				if (containerRect.height !== newContainerRect.height) {
					this._heightAnimation = this.ref.container.animate([
						{ height: `${containerRect.height}px` },
						{ height: `${newContainerRect.height}px` }
					], {
						duration: 400,
...
gia.register(FilterableListClassic);
...
<div data-component="FilterableListClassic" data-options='{"defaultSort": "name:asc"}'>
  <div class="controls">
    <!-- Filter Controls -->
...
div[data-component="FilterableListClassic"] {
  .grid {
    display: grid;
