class UploadField extends gia.Component {
	...
	handleFocus(event) {
		const dropzone = event.target.closest('[data-component="UploadField"]') || event.target.closest('.form-dropzone');
		if (dropzone) dropzone.classList.add('is-focused');
	}

	handleBlur(event) {
		const dropzone = event.target.closest('[data-component="UploadField"]') || event.target.closest('.form-dropzone');
		if (dropzone) dropzone.classList.remove('is-focused');
	}
