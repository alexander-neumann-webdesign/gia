...
export default class Component {
	...
	loadScript(scriptId, globalName) {
		...
		script._loadPromise = new Promise((resolve, reject) => {
			...
			// ⚡ BOLT OPTIMIZATION: Use getAttribute instead of dataset for faster DOM attribute access and to avoid DOMStringMap allocation
			const dataSrc = script.getAttribute("data-src");
			if (!script.src && dataSrc) {
				script.src = dataSrc;
				// Clean up the data attribute to keep DOM tidy (optional)
				script.removeAttribute("data-src");
			} else if (!script.src && !dataSrc) {
	...
	loadStyle(styleId) {
		...
		link._loadPromise = new Promise((resolve, reject) => {
			...
			if (!link.href && dataHref) {
				link.href = dataHref;
				// Clean up the data attribute to keep DOM tidy (optional)
				link.removeAttribute("data-href");
			} else if (!link.href && !dataHref) {
				// Edge case: Tag exists but has no source at all
				cleanup();
