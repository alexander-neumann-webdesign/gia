...
## 2025-12-11 - Generic Dot Button Tooltips
**Learning:** Found an accessibility issue pattern specific to this app's components - while generic dot buttons (like slider pagination in `Slider.js`) are dynamically created with an `aria-label` attribute for screen readers, they lack a native `title` attribute. Sighted mouse users do not get context without a visual tooltip for these abstract UI elements.
**Action:** Always add a native `title` attribute to generic dot buttons (like slider pagination) or icon-only buttons to provide a built-in visual tooltip for mouse hover, serving as a necessary complement to the `aria-label`.
