...
## 2024-06-09 - Avoid GC Churn in High-Frequency DOM Observer Callbacks
**Learning:** Instantiating new data structures (like `Set` or `Map`) directly inside high-frequency callback functions such as `MutationObserver` creates unnecessary intermediate allocations. This rapidly accumulates garbage, leading to unpredictable garbage collection churn that manifests as micro-stutters during heavy DOM manipulation or animations. However, if these collections track DOM elements, failing to clear them *after* processing will retain strong references to detached elements, causing a memory leak.
**Action:** Extract temporary tracking collections to the module scope (or class property) when used within `MutationObserver`, `IntersectionObserver`, `ResizeObserver`, `requestAnimationFrame`, or general scrolling/resizing handlers. Re-use the existing collection by calling `.clear()` at the start of each callback execution instead of allocating a `new Set()` each time. Crucially, always `.clear()` the collection again at the *end* of the callback if it holds references to DOM elements to prevent memory leaks.
