...
## 2024-07-16 - Extract Array Normalization in RangeSlider
**Learning:** Extracting repeated ternary array normalization logic (e.g. `Array.isArray(ref) ? ref : (ref ? [ref] : [])`) into a reusable class method like `_getArray()` improves code DRY-ness and maintainability without impacting the self-contained nature of example components.
**Action:** Always prefer extracting verbose ternary logic that operates on `this.ref` DOM references into a helper method, assigning the normalized array directly to the property (e.g., `this.ref.inputs = this._getArray(this.ref.inputs);`).
