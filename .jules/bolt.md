...
## 2026-06-29 - Extract monolithic logic into smaller helper functions
**Learning:** Component methods that handle massive sequential logic operations (such as the `_animateTransition` in the Tabs component, which orchestrates DOM manipulations, setup for animations, multiple WAAPI animations simultaneously, and cleanup logic) can become complex and difficult to maintain. Breaking these apart into smaller, distinct helper methods (e.g. `_setupPanelsForAnimation`, `_createAnimations`, `_cleanupAfterAnimation`) significantly improves code readability and maintainability while keeping the overarching animation logic flow clean.
**Action:** Always attempt to split complex, monolithic functions into smaller, single-purpose helper functions within the same class, allowing subclasses or future extensions to reuse or override distinct parts of the logic without rewriting everything.
