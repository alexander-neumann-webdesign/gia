...
## 2024-05-15 - Fix memory leak and lag caused by .forEach on mutated array
**Learning:** Using `Array.prototype.forEach()` on an array that uses a swap-and-pop removal pattern (like `this._observedScrollCallbacks.forEach(this.unobserveScroll)`) causes bugs where half the elements are skipped. Because elements are swapped from the end into the current index and then popped, `.forEach` evaluates the skipped elements and the length reduces, prematurely stopping the iteration. This causes memory leaks and degrading scroll/resize performance over time because listeners are never fully removed.
**Action:** Always use backward standard `for` loops (e.g., `for (let i = arr.length - 1; i >= 0; i--)`) when iterating over arrays that remove elements from themselves during the loop, ensuring all items are correctly processed and removed.
