...
## 2024-06-06 - Prevent DOM Clobbering Using `window` Identity
**Vulnerability:** The existing `instanceof Window` check was failing to accurately identify injected `iframe` elements as the Window object on the `window` prototype chain under specific browser/JSDOM context environments, allowing attackers to bypass `loadScript` safety checks.
**Learning:** `instanceof Window` can be spoofed or unreliable across different global execution contexts (especially when iframes create separate execution environments with their own `Window` constructors). Comparing `window[globalName] !== window[globalName].window` accurately determines if the object is a Window because only true `Window` objects have a circular `window` reference pointing to themselves.
**Prevention:** When protecting against DOM clobbering targeting `window` properties, use the circular reference check `window[prop].window === window[prop]` to definitively identify Window objects instead of relying on `instanceof`.
