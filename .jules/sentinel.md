...
## 2025-02-18 - Prevent DOM-based Prototype DoS
**Vulnerability:** DOM-based Denial of Service and Prototype Pollution via Object references.
**Learning:** Initializing objects with `{}` or directly checking keys via `obj[userKey]` allows users to inject values like `__proto__`, `constructor`, or `hasOwnProperty` via DOM attributes (e.g. `data-ref`, `data-component`). Accessing these causes native prototype lookups, throwing TypeErrors (like "is not a constructor" or "list.push is not a function") when the app expects components or arrays, ultimately crashing the execution context.
**Prevention:** Always initialize dictionary objects bound to user input with `Object.create(null)` to prevent prototype inheritance. Validate properties using `Object.prototype.hasOwnProperty.call(obj, key)` instead of relying on `in` or truthy checks.
