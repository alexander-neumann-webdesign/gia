...
## 2026-05-25 - DOM Clobbering in loadScript method
...
## 2025-02-14 - DOM-based XSS via insertAdjacentHTML
**Vulnerability:** XSS vulnerabilities found in multiple example components (`Accordion.js`, `Form.js`, `QRCode.js`, `VideoHolder.js`) due to the use of `insertAdjacentHTML`. While in some cases the input was hardcoded strings, in others it could be influenced by component state or options (e.g., `qr.createSvgTag()`).
**Learning:** `insertAdjacentHTML` parses strings as HTML and executes scripts if present, making it a common vector for DOM-based XSS if the string contains any user-controllable data.
**Prevention:** Avoid `insertAdjacentHTML` and `innerHTML` whenever possible. Use safer DOM APIs like `document.createElement` (or `document.createElementNS` for SVG elements), set properties/attributes directly, and append the elements using `appendChild` or `insertBefore`.
## 2025-02-14 - DOM-based XSS via insertAdjacentHTML
**Vulnerability:** XSS vulnerabilities found in multiple example components (`Accordion.js`, `Form.js`, `QRCode.js`, `VideoHolder.js`) due to the use of `insertAdjacentHTML`. While in some cases the input was hardcoded strings, in others it could be influenced by component state or options (e.g., `qr.createSvgTag()`). Note that `template.innerHTML` is also unsafe for the same reasons.
**Learning:** `insertAdjacentHTML` and `innerHTML` parses strings as HTML and executes scripts if present, making it a common vector for DOM-based XSS if the string contains any user-controllable data.
**Prevention:** Avoid `insertAdjacentHTML` and `innerHTML` whenever possible. Use safer DOM APIs like `document.createElement` (or `document.createElementNS` for SVG elements), `DOMParser.parseFromString` for string parsing without execution, set properties/attributes directly, and append the elements using `appendChild` or `insertBefore`.
