...
## 2026-05-27 - DOM Clobbering Bypass via tagName property in MultiStepForm Validation
**Vulnerability:** A check in `examples/MultiStepForm.js` relied on `indicator.tagName === 'BUTTON'` to determine if a progress indicator was a button element. This approach is vulnerable to DOM Clobbering. An attacker could potentially inject elements (like an `<input name="tagName">` inside a form) that would overwrite the `tagName` property, thus bypassing this type check and potentially causing the script to perform unsafe operations on the wrong element type.
**Learning:** Checking `element.tagName` can be clobbered. Logic relying on it can be bypassed if the DOM structure allows nested elements with matching `name` attributes, especially inside forms.
**Prevention:** Use strictly typed `instanceof` checks (e.g., `element instanceof HTMLButtonElement`) rather than relying on the `tagName` property when verifying the type of DOM elements.
