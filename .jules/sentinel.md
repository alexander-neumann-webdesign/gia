...
## 2026-05-27 - [Security] Prevent DOM-based XSS by replacing insertAdjacentHTML in Accordion
...
## 2026-05-27 - [Security] Prevent DOM-based XSS via clipboard fallback
**Vulnerability:** The ClipboardCopy component used `document.execCommand('copy')` on a dynamically created and appended `<textarea>` containing user-supplied text. The original implementation allowed DOM insertion of arbitrary strings. While modern browsers sanitize `value` property assignments on textareas, creating hidden textareas to execute `copy` commands can sometimes lead to unexpected injection vectors or clobbering depending on how the input text is derived.
**Learning:** Using legacy `execCommand` with temporary DOM nodes (like hidden textareas) is an outdated pattern that introduces unnecessary DOM manipulation for tasks that should remain purely in Javascript memory, expanding the attack surface.
**Prevention:** Avoid legacy DOM-based clipbaord fallbacks (`document.execCommand`). Modern applications should rely exclusively on the asynchronous `navigator.clipboard.writeText()` API. If it fails, degrade gracefully (e.g. show an error state) instead of injecting temporary nodes into the DOM.
